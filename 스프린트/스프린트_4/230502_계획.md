		...
		- Edit
			1. Visit
				...
				- Fields
					1. Name *
					2. Number * (중복 안됨)
					4. Type : Scheduled(선택하지 못함, 이미 이 값을 가진 경우 변경 불가능)|Unscheduled|Common *
					5. Window
					6. \-
					7. \+
					8. Repeating : Yes|No(디폴트)
					9. Mandatory : Yes|No(디폴트)
...
### DCI 메뉴 분리
1. 순서 : CRF - EDC 선택 - ODM 편집 (여기까지 워크스페이스) | (여기부터 Log, 스냅샷) - 전송 - 전송 히스토리
2. 기준 : ~~BOIM 우리 안하기로 했는데!~~ 엑셀로 주고받는다?
3. ODM 형태가 있어야 테이블로 만들 수 있음 = ODM 편집 화면 필요
4. CRF : DCI = 1 : 1
5. 위치 : CRF 이후 SDTM 이전, 어차피 SDTM 하나라 했으니까 SDTM 위에다가 하자
